...
def create_and_save_tensorboard_callback(callbacks_dir, tensorboard_log_dir):
    ...
    tb_calling_filepath = os.path.join(callbacks_dir, 'tensorboard_cb.cb')
    joblib.dump(tensorboard_callback, tb_calling_filepath)
    logging.info(f'tensorboard callbacks is being saved at {tb_calling_filepath}')
